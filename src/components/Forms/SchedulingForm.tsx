...
export default function SchedulingForm({
  ...
  const handleNext = (e: React.FormEvent) => {
    ...
    // ⬇️ Save schedule and rental info
    ...
    // ⬇️ Create dealer courtesy note if selected
    if (rentalCompany === "Dealer Courtesy") {
      localStorage.setItem("rental1", "courtesy"); // ⬅️ this is critical
      localStorage.setItem(
        "dealerCourtesyNote",
        "Dealer Courtesy Required. HMA Customer will receive a courtesy vehicle from the dealership."
      );
    } else {
      localStorage.setItem("rental1", rental1); // restore original yes/no
